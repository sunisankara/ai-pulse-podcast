...
async function run() {
  console.log('--- AI DAILY PULSE BROADCASTER (v0.6.4) ---');
  ...
  try {
    if (IS_TEST) {
      ...
      console.log('Step 1: Dynamic News Research...');
      const news = await fetchAINews(["LLM Breakthroughs", "AI Startup Business Moves", "New Dev Tooling"], true);
      console.log('Step 2: Natural Conversation Scripting...');
      const script = await generatePodcastScript(news.newsText);
      console.log('Step 3: Multi-Speaker Production...');
      const segments = script.split('[TRANSITION]').filter(s => s.trim().length > 5);
      const pcmFile = `temp-${id}.pcm`;
      for (const seg of segments) {
        const chunks = await generateSegmentAudio(seg);
