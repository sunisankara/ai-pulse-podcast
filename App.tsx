...
const App: React.FC = () => {
  ...
  const handleTestConnection = async () => {
    if (!githubUser || !githubRepo || !ghToken) {
      setError("Configuration missing. Please provide GitHub Username, Repo, and Token.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`GitHub API Error: ${data.message || 'Unknown Error'}. Ensure token has "repo" access.`);
      }

      setStep(GenerationStep.COMPLETED);
      setProgress('Diagnostic workflow successfully triggered on GitHub. Redirecting to logs...');
      setTimeout(() => {
    ...
    <div className="max-w-7xl mx-auto px-4 py-10 font-sans">
      ...
      <div className="flex flex-wrap items-center justify-between gap-6 mb-12 bg-gray-800 p-8 rounded-[3rem] border border-gray-700 shadow-2xl">
        ...
        <div className="text-right flex items-center gap-4">
           <button onClick={handleTestConnection} className="bg-amber-500 hover:bg-amber-400 text-black px-6 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all shadow-lg shadow-amber-900/20 active:scale-95">
             <i className="fas fa-plug-circle-bolt mr-2"></i> Test Cloud Connection
           </button>
          <span className="text-[10px] font-black bg-blue-500/10 text-blue-400 px-4 py-4 rounded-2xl border border-blue-500/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase block">GitHub Access Token (PAT)</label>
                  <button onClick={() => setShowTokenGuide(!showTokenGuide)} className="text-[10px] text-blue-400 hover:text-blue-300 font-bold uppercase tracking-tighter">
                    {showTokenGuide ? 'Close Guide' : 'How to get this?'}
                  </button>
                </div>
                
                {showTokenGuide && (
                  <div className="mb-4 bg-blue-900/20 border border-blue-500/30 rounded-2xl p-6 text-[11px] leading-relaxed text-blue-200 animate-in fade-in slide-in-from-top-4 duration-300">
                    <p className="font-bold mb-3 uppercase text-blue-400">Step-by-Step Token Guide:</p>
                    <ol className="list-decimal list-inside space-y-2">
                      ...
                      <li><b>CRITICAL:</b> Check the <b>"repo"</b> scope checkbox.</li>
                      <li>Generate and <b>COPY</b> the token immediately.</li>
                    </ol>
                  </div>
                )}
                ...
                <p className="text-[9px] text-gray-500 mt-2 px-2 italic">Required for triggering the remote diagnostic test and uploading the 5m test tone.</p>
              </div>
              ...
              <div className="bg-blue-600/10 p-8 rounded-[2rem] border border-blue-500/20">
                ...
                <div className="flex gap-4">
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(rssLink); setProgress('Link Copied'); setTimeout(()=>setProgress(''), 2000); }} className="bg-blue-600 px-6 rounded-xl hover:bg-blue-500 transition-all font-black uppercase text-[10px]">Copy</button>
                </div>
          ...
          <div className="bg-gray-800 rounded-[3rem] p-12 border border-gray-700">
            ...
            {(step !== GenerationStep.IDLE || error) && (
               <div className={`mt-8 p-6 rounded-3xl border ${error ? 'bg-red-500/10 border-red-500/40' : 'bg-black/40 border-blue-500/20 animate-pulse'}`}>
                  <p className={`text-[10px] font-black uppercase ${error ? 'text-red-500' : 'text-blue-400'}`}>{error ? 'Connection Fault' : step}</p>
        ...
        <div className="lg:col-span-5 space-y-8">
          <div className="bg-blue-600 rounded-[3rem] p-10 border border-blue-400 shadow-2xl text-white">
            <h2 className="text-3xl font-black italic tracking-tighter mb-6 uppercase">Setup Checklist</h2>
            <div className="space-y-6 text-sm">
              ...
              <div className="flex gap-4">
                ...
                <p>Upload <code>headless.ts</code>, <code>types.ts</code>, and <code>podcast.yml</code> to your repo.</p>
              </div>
              <div className="flex gap-4">
                ...
                <p>Run <b>Test Cloud Connection</b> to upload the 5m monotone MP3 diagnostic.</p>
              </div>
            </div>
          </div>
